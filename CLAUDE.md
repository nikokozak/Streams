...
## Alpha Posture (Proxy-Only) — Read Before AI Work

- Proxy-only is the alpha default (`SettingsService.proxyOnlyMode == true`): no local OpenAI/Anthropic/Perplexity calls and no vendor keys stored.
- Stream cell AI output contract is explicit and must not be “accidental”:
  - The model returns Markdown; the web layer converts Markdown → sanitized HTML on completion.
  - Option A restatement: the response begins with a Markdown heading (`## <topic>`) followed by the body (single request; no separate restatement call).
- Canonical execution plan (Option A + house-cleaning audit): `docs/ALPHA_STABILITY_PLAN.md`.
...
| `Services/AIOrchestrator.swift` | Routes all LLM via proxy; attaches intent for proxy routing |
| `Services/ProxyLLMService.swift` | **Primary** proxy LLM client (SSE streaming + error mapping + vision parts) |
| `Services/AIService.swift` | Legacy vendor client (must not run in proxy-only alpha) |
| `Services/AnthropicService.swift` | Legacy vendor client (must not run in proxy-only alpha) |
| `Services/PerplexityService.swift` | Legacy vendor client (must not run in proxy-only alpha) |
...
| `Services/EmbeddingService.swift` | OpenAI embeddings (disabled in proxy-only alpha; RAG deferred) |
| **Models** | |
...
- Serial/device key is stored **only** in Application Support (Swift-side); never persist it in Web storage or logs.
